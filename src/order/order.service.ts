...
export class OrderService {
  ...
  async create(createOrderDto: any, @Req() req): Promise<any> {
    ...
    for (const product of products as { productSkuId: string; quantity: number }[]) {
      const existingProductSku = await this.prisma.productSku.findUnique({
        where: { id: product.productSkuId },
      });
  
      if (!existingProductSku) {
        throw new NotFoundException(`Product SKU ID ${product.productSkuId} not found.`);
      }
    }
